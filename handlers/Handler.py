...
class Handler(webapp2.RequestHandler):
	# I don't think method is ever used and I'm not sure what it does
    def dispatch(self):
    ...
    def render(self, template, **kw):
        ...
        try:
            ...
            # adds up the three different types of notifications
            notification_count = user.passenger_requests.count() + \
				user.driver_responses.count() + \
				len(user.payment_notifications)
            ...
            if self.session.get('signed_into_venmo'):
                ...
                self.write(self.render_str(template, username=username,
					token=self.session.get('channel_token'),
					notification_count=notification_count,
					CLIENT_ID=constants.CLIENT_ID,
					**kw))
    ...
    def deleteOldRides(self):
        ...
        rides = Ride.all().filter("startTime < ", now)
        for ride in rides:
            ride.archive()

    def sendActivationEmail(self, email, code):
		"""Sends an email with an activation code so the user can activate their account"""
    ...
    def getVerifyURL(self, code):
		"""Formats a code into a verification URL"""
		return "http://%s/%s?code=%s" % (self.request.host, 'verify', code)

    def getLocationInfo(self, start, destination):
		"""Gets information about the route between two geocode objects"""
    ...
    def payForRide(self, ride, passenger):
		"""Sends a payment from passenger to driver and links the passenger with the ride"""
