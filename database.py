...
class User(db.Model):
	...
	# Any ride the user is a part of (driver or passenger)
	rideIds = db.ListProperty(int)

	# Adds a ride that this user is a part of
	def addRide(self, ride):
		self.rideIds.append(int(ride.key().id()))

	# Removes the user from this ride
	...
	# Returns a list of all the rides the user is a part of
	@property
	def rides(self):
		ret = [Ride.get_by_id(ride) for ride in self.rideIds]
		return [r for r in ret if r is not None]

	# Deactivates an account **Should this also archive?**
	def archive(self):
...
class Payment(db.Model):
	...
	def put(self):
		...
		# add a PassengerRequestNotification to driverId
		paymentNotification = PaymentNotification(payment=self, driver=self.driver,
				passenger=self.passenger, status=self.status)
